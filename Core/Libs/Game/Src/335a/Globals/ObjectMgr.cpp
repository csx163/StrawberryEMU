...
void ObjectMgr::LoadQuestAreaTriggers()
{
    ...
    do
    {
        ...
        AreaTriggerData const* atEntry = GetAreaTriggerData(trigger_ID);
        if (!atEntry)
        {
...
void ObjectMgr::LoadAccessRequirements()
{
    ...
    do
    {
        ...
        if (ar.achievement)
        {
            if (!sObjectMgr->GetAchievementData(ar.achievement))
            {
...
AreaTrigger const* ObjectMgr::GetGoBackTrigger(uint32 Map) const
{
    ...
    for (AreaTriggerMap::const_iterator itr = mAreaTriggers.begin(); itr != mAreaTriggers.end(); ++itr)
        ...
        {
            AreaTriggerData const* atEntry = sObjectMgr->GetAreaTriggerData(itr->first);
...
/*****************************DBC AND DB2 DATA*****************************/
AchievementData const* ObjectMgr::GetAchievementData(uint32 AchievementId)
{
    AchievementDataContainer::const_iterator itr = sData->AchievementDataTable.find(AchievementId);
    if (itr != sData->AchievementDataTable.end())
        return &(itr->second);

    return NULL;
}

AchievementCategoryData const* ObjectMgr::GetAchievementCategoryData(uint32 AchievementId)
{
    AchievementCategoryDataContainer::const_iterator itr = sData->AchievementCategoryDataTable.find(AchievementId);
    if (itr != sData->AchievementCategoryDataTable.end())
        return &(itr->second);

    return NULL;
}

AchievementCriteriaData const* ObjectMgr::GetAchievementCriteriaData(uint32 AchievementId)
{
    AchievementCriteriaDataContainer::const_iterator itr = sData->AchievementCriteriaDataTable.find(AchievementId);
    if (itr != sData->AchievementCriteriaDataTable.end())
        return &(itr->second);

    return NULL;
}

AreaGroupData const* ObjectMgr::GetAreaGroupData(uint32 AreaGroupId)
{
    AreaGroupDataContainer::const_iterator itr = sData->AreaGroupDataTable.find(AreaGroupId);
    if (itr != sData->AreaGroupDataTable.end())
        return &(itr->second);

    return NULL;
}

AreaTableData const* ObjectMgr::GetAreaTableData(uint32 AreaId)
{
    AreaTableDataContainer::const_iterator itr = sData->AreaTableDataTable.find(AreaId);
    if (itr != sData->AreaTableDataTable.end())
        return &(itr->second);

    return NULL;
}

AreaTriggerData const* ObjectMgr::GetAreaTriggerData(uint32 TriggerId)
{
