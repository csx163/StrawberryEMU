...
bool Player::CanJoinConstantChannelInZone(ChatChannelsEntry const* channel, AreaTableData const* zone)
{
...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    if (!(currFields & val))
    {
        ...
        AreaTableData const *p = GetAreaEntryByAreaFlagAndMap(areaFlag, GetMapId());
        if (!p)
        {
        ...
        else if (p->AreaLevel > 0)
        {
            ...
            else
            {
                ...
                else if (diff > 5)
                {
                    ...
                    XP = uint32(sObjectMgr->GetBaseXP(p->AreaLevel)*exploration_percent/100*sWorld->getRate(RATE_XP_EXPLORE));
                }
                else
                {
                    XP = uint32(sObjectMgr->GetBaseXP(p->AreaLevel)*sWorld->getRate(RATE_XP_EXPLORE));
                }
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        if (ar->achievement && !GetAchievementMgr().HasAchieved(sObjectMgr->GetAchievementData(ar->achievement)))
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, sObjectMgr->GetItemTemplate(missingItem)->Name1.c_str());
                else if (LevelMin)
...
void Player::CompletedAchievement(AchievementData const* entry, bool ignoreGMAllowAchievementConfig)
{
