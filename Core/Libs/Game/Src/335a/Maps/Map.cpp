...
inline bool IsOutdoorWMO(uint32 mogpFlags, int32 /*adtId*/, int32 /*rootId*/, int32 /*groupId*/, WMOAreaTableEntry const* wmoEntry, AreaTableData const* atEntry)
{
    bool outdoor = true;
...
{
    AreaTableData const *entry = GetAreaEntryByAreaFlagAndMap(areaflag, map_id);
...
uint32 Map::GetZoneIdByAreaFlag(uint16 areaflag, uint32 map_id)
{
    ...
    if (entry)
        return (entry->ZoneId != 0) ? entry->ZoneId : entry->Id;
    else
        return 0;
}
...
void Map::GetZoneAndAreaIdByAreaFlag(uint32& zoneid, uint32& areaid, uint16 areaflag, uint32 map_id)
{
    ...
    zoneid = entry ? ((entry->ZoneId != 0) ? entry->ZoneId : entry->Id) : 0;
}
