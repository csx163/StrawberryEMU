...
bool ChatHandler::HandleGPSCommand(const char* args)
{
    ...
    PSendSysMessage(LANG_MAP_POSITION,
        ...
        zone_id, (zoneEntry ? zoneEntry->AreaName : "<unknown>"),
        area_id, (areaEntry ? areaEntry->AreaName : "<unknown>"),
        obj->GetPhaseMask(),
...
bool ChatHandler::HandleLookupAreaCommand(const char* args)
{
    ...
    for (uint32 areaflag = 0; areaflag < (uint32)sObjectMgr->GetAreaTableData(areaflag); ++areaflag)
    {
        AreaTableData const *areaEntry = sObjectMgr->GetAreaTableData(areaflag);
        if (areaEntry)
        {
