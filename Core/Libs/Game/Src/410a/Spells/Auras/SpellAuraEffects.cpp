    ...
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_ADD_PCT_MODIFIER:
            if (!m_spellmod)
            {
                ...
                m_spellmod->spellId = GetId();
                m_spellmod->mask = (flag96)spellEffect->EffectSpellClassMaskA[GetEffIndex()];
...
bool AuraEffect::IsAffectedOnSpell(SpellEntry const *spell) const
{
    ...
    // Check family name
    if (classOptions->SpellFamilyName != classOptions->SpellFamilyName)
        return false;

    // Check EffectClassMask
    if (spellEffect->EffectSpellClassMaskA[m_effIndex] & classOptions->SpellFamilyFlags)
        return true;
    return false;
}
...
void AuraEffect::HandleNoReagentUseAura(AuraApplication const * aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    for (Unit::AuraEffectList::const_iterator i = noReagent.begin(); i != noReagent.end(); ++i)
        mask |= (flag96)(*i)->m_spellEffect->EffectSpellClassMaskA[SpellEffIndex((*i)->m_effIndex)];
