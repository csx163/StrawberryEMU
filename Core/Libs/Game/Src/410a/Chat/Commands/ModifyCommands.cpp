...
public:
    ...
    static bool HandleModifyHonorCommand (ChatHandler* handler, const char* args)
    {
        ...
        handler->PSendSysMessage(LANG_COMMAND_MODIFY_HONOR, handler->GetNameLink(target).c_str(), 0);
    ...
    static bool HandleModifyArenaCommand(ChatHandler* handler, const char* args)
    {
        ...
        handler->PSendSysMessage(LANG_COMMAND_MODIFY_ARENA, handler->GetNameLink(target).c_str(), 0);
