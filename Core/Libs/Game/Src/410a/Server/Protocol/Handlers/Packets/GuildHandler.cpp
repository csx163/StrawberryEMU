...
void WorldSession::HandleGuildSetOfficerNoteOpcode(WorldPacket& recvPacket)
{
    ...
    if (Guild* pGuild = _GetPlayerGuild(this->GetPlayer()->GetSession(), true))
        pGuild->HandleSetMemberNote(this->GetPlayer()->GetSession(), guid, Note, !ispublic);
}
...
void WorldSession::HandleGuildBankDepositMoney(WorldPacket & recv_data)
{
    ...
    if (GetPlayer()->GetGameObjectIfCanInteractWith(GoGuid, GAMEOBJECT_TYPE_GUILD_BANK))
        if (money && GetPlayer()->HasEnoughMoney(money))
            if (Guild* pGuild = _GetPlayerGuild(this))
                pGuild->HandleMemberDepositMoney(this, uint32(money));
}
...
void WorldSession::HandleGuildBankWithdrawMoney(WorldPacket & recv_data)
{
    ...
    if (money)
        if (GetPlayer()->GetGameObjectIfCanInteractWith(GoGuid, GAMEOBJECT_TYPE_GUILD_BANK))
            if (Guild* pGuild = _GetPlayerGuild(this))
                pGuild->HandleMemberWithdrawMoney(this, uint32(money));
}
