...
inline bool IsSpellHaveEffect(SpellEntry const *spellInfo, SpellEffects effect)
{
    for (int i= 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        if (SpellEffects(spellEffect->Effect) == effect)
            return true;
    }
    return false;
}
...
inline bool IsSpellHaveAura(SpellEntry const *spellInfo, AuraType aura)
{
    for (int i= 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        if (AuraType(spellEffect->EffectApplyAuraName) == aura)
            return true;
    }
    return false;
}
...
inline bool IsElementalShield(SpellEntry const *spellInfo)
{
    ...
    return (classOptions->SpellFamilyFlags & 0x420
...
inline bool IsExplicitDiscoverySpell(SpellEntry const *spellInfo)
{
    ...
    SpellEffectEntry const* spellEffect_1 = spellInfo->GetSpellEffect(SpellEffIndex(EFFECT_1));
    if (!spellEffect || !spellEffect_1)
        return false;

    return (((spellEffect->Effect == SPELL_EFFECT_CREATE_RANDOM_ITEM
        || spellEffect->Effect == SPELL_EFFECT_CREATE_ITEM_2)
        && spellEffect_1->Effect == SPELL_EFFECT_SCRIPT_EFFECT)
...
inline bool IsLootCraftingSpell(SpellEntry const *spellInfo)
{
    ...
    if (!spellEffect || !totems)
        return false;

    return (spellEffect->Effect == SPELL_EFFECT_CREATE_RANDOM_ITEM ||
        // different random cards from Inscription (121==Virtuoso Inking Set category) or without explicit item
        (spellEffect->Effect == SPELL_EFFECT_CREATE_ITEM_2 &&
        (totems->TotemCategory[0] != 0 || spellEffect->EffectItemType == 0)));
}
...
inline bool IsAreaOfEffectSpell(SpellEntry const *spellInfo)
{
    SpellEffectEntry const* spellEffect = spellInfo->GetSpellEffect(SpellEffIndex(EFFECT_0));
    SpellEffectEntry const* spellEffect_1 = spellInfo->GetSpellEffect(SpellEffIndex(EFFECT_1));
    SpellEffectEntry const* spellEffect_2 = spellInfo->GetSpellEffect(SpellEffIndex(EFFECT_2));
    if (!spellEffect|| !spellEffect_1 || !spellEffect_2)
        return false;

    if (IsAreaEffectTarget[spellEffect->EffectImplicitTargetA] || IsAreaEffectTarget[spellEffect->EffectImplicitTargetB])
        return true;
    if (IsAreaEffectTarget[spellEffect_1->EffectImplicitTargetA] || IsAreaEffectTarget[spellEffect_1->EffectImplicitTargetB])
        return true;
    if (IsAreaEffectTarget[spellEffect_2->EffectImplicitTargetA] || IsAreaEffectTarget[spellEffect_2->EffectImplicitTargetB])
        return true;
    return false;
}
...
inline bool IsDispel(SpellEntry const *spellInfo)
{
    ...
    if (spellEffect->Effect == SPELL_EFFECT_DISPEL ||
        spellEffect_1->Effect == SPELL_EFFECT_DISPEL ||
        spellEffect_2->Effect == SPELL_EFFECT_DISPEL)
        return true;
    return false;
}
...
inline bool IsDispelSpell(SpellEntry const *spellInfo)
{
    ...
    //spellsteal is also dispel
    if (spellEffect->Effect == SPELL_EFFECT_STEAL_BENEFICIAL_BUFF ||
        spellEffect_1->Effect == SPELL_EFFECT_STEAL_BENEFICIAL_BUFF ||
        spellEffect_2->Effect == SPELL_EFFECT_STEAL_BENEFICIAL_BUFF ||
        IsDispel(spellInfo))
        return true;
    return false;
}
...
inline bool IsRangedWeaponSpell(SpellEntry const* spellInfo)
{
    ...
    if (!equippedItems || !classOptions)
        return false;

    //spell->DmgClass == SPELL_DAMAGE_CLASS_RANGED should be checked outside
    return (spellInfo->GetSpellFamilyName() == SPELLFAMILY_HUNTER && !(classOptions->SpellFamilyFlags & 0x10000000)) // for 53352, cannot find better way
        || (equippedItems->EquippedItemSubClassMask & ITEM_SUBCLASS_MASK_WEAPON_RANGED);
}
...
inline uint32 GetAllSpellMechanicMask(SpellEntry const* spellInfo)
{
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        if (spellEffect->Effect && spellEffect->EffectMechanic)
            mask |= 1 << spellEffect->EffectMechanic;
    }
    return mask;
}
...
inline Mechanics GetEffectMechanic(SpellEntry const* spellInfo, int32 effect)
{
    ...
    if (spellEffect->EffectMechanic)
        return Mechanics(spellEffect->EffectMechanic);
    if (spellInfo->GetMechanic())
