...
void Spell::EffectResurrectNew(SpellEffectEntry const* effect)
{
    ...
    uint32 mana = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
        ...
        switch (m_spellInfo->GetSpellFamilyName())
        {
            case SPELLFAMILY_GENERIC:
            {
                ...
                switch(m_spellInfo->Id)                     // better way to check unknown
                {
                    ...
                    // Shadowbolts only affects targets with Shadow Mark (Gothic)
                    case 27831:
                    case 55638:
            ...
            case SPELLFAMILY_WARLOCK:
            {
                ...
                else if (m_spellInfo->GetTargetAuraState() == AURA_STATE_CONFLAGRATE)
                {
                    ...
                    SpellEffectEntry const* spellEffect = aura->GetSpellProto()->GetSpellEffect(SpellEffIndex(EFFECT_0));
                    if (!spellEffect)
                        return;
                    ...
                    if (aura)
                    {
                        ...
                        uint8 baseTotalTicks = uint8(m_caster->CalcSpellDuration(aura->GetSpellProto()));
...
{
    if (m_spellInfo->GetEffectMiscValue(SpellEffIndex(i)))
        speedZ = float(m_spellInfo->GetEffectMiscValue(SpellEffIndex(i)))/10;
    else if (m_spellInfo->GetEffectMiscValueB(SpellEffIndex(i)))
        speedZ = float(m_spellInfo->GetEffectMiscValueB(SpellEffIndex(i)))/10;
    else
        speedZ = 10.0f;
...
{
    if (m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)) < 0 || m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)) >= int8(MAX_POWERS))
        return;

    Powers powerType = Powers(m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)));
...
void Spell::EffectSendEvent(SpellEffectEntry const* effect)
{
    ...
    */
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell ScriptStart %u for spellid %u in EffectSendEvent ", m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)), m_spellInfo->Id);
    ...
    if (unitTarget)
    {
        if (ZoneScript* zoneScript = unitTarget->GetZoneScript())
            zoneScript->ProcessEvent(unitTarget, m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)));
    }

    m_caster->GetMap()->ScriptsStart(sEventScripts, m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)), m_caster, pTarget);
}
...
void Spell::SpellDamageHeal(SpellEffectEntry const* /*effect*/)
{
    ...
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->GetTargetAuraState() == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND, m_spellInfo, m_caster))
        {
            ...
            SpellClassOptionsEntry const* classOptions = targetAura->GetSpellProto()->GetSpellClassOptions();
...
void Spell::EffectEnergize(SpellEffectEntry const* effect)
{
    ...
    Powers power = Powers(m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)));
...
void Spell::EffectOpenLock(SpellEffectEntry const* effect)
{
    ...
    ExecuteLogEffectOpenLock(uint8(effect->EffectIndex), gameObjTarget ? (Object*)gameObjTarget : (Object*)itemTarget);
}
...
{
    SpellEquippedItemsEntry const* equippedItems = m_spellInfo->GetSpellEquippedItems();
    if (!unitTarget || !equippedItems || unitTarget->GetTypeId() != TYPEID_PLAYER)
        return;
    Player *p_target = (Player*)unitTarget;

    uint32 subClassMask = equippedItems->EquippedItemSubClassMask;
    if (equippedItems->EquippedItemClass == ITEM_CLASS_WEAPON && !(p_target->GetWeaponProficiency() & subClassMask))
    {
        p_target->AddWeaponProficiency(subClassMask);
        p_target->SendProficiency(ITEM_CLASS_WEAPON, p_target->GetWeaponProficiency());
    }
    if (equippedItems->EquippedItemClass == ITEM_CLASS_ARMOR && !(p_target->GetArmorProficiency() & subClassMask))
    {
...
{
    uint32 entry = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
    if (!entry)
        return;

    SummonPropertiesEntry const *properties = sSummonPropertiesStore.LookupEntry(m_spellInfo->GetEffectMiscValueB(SpellEffIndex(effect->EffectIndex)));
    if (!properties)
    {
        sLog->outError("EffectSummonType: Unhandled summon type %u", m_spellInfo->GetEffectMiscValueB(SpellEffIndex(effect->EffectIndex)));
        return;
    }
...
void Spell::EffectDispel(SpellEffectEntry const* effect)
{
    ...
    uint32 dispel_type = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectLearnSkill(SpellEffectEntry const* effect)
{
    ...
    uint32 skillid =  m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectEnchantItemPerm(SpellEffectEntry const* effect)
{
    ...
    else
    {
        ...
        uint32 enchant_id = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
        if (!enchant_id)
            return;
...
void Spell::EffectSummonPet(SpellEffectEntry const* effect)
{
    ...
    uint32 petentry = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));

    if (!owner)
    {
        SummonPropertiesEntry const *properties = sSummonPropertiesStore.LookupEntry(67);
        if (properties)
            SummonGuardian(effect->EffectIndex, petentry, properties);
        return;
    }
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (classOptions->SpellFamilyFlags & 0x20000)
            {
                ...
                if (AuraEffect const * aurEff = m_caster->GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 2710, EFFECT_0))
                    // Do not consume diseases if roll success
                    if (roll_chance_i(aurEff->GetAmount()))
...
{
    uint32 gameobject_id = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectScriptEffect(SpellEffectEntry const* effect)
{
                ...
                case 64142:                                 // Upper Deck - Create Foam Sword
                    ...
                    for (uint32 const *itr = &itemId[0]; *itr; ++itr)
                        if (plr->HasItemCount(*itr, 1, true))
                            return;
                    DoCreateItem(effect->EffectIndex, itemId[urand(0,4)]);
                    return;
            }
            break;
        }
    ...
    // normal DB scripted effect
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell ScriptStart spellid %u in EffectScriptEffect(%u)", m_spellInfo->Id, effect->EffectIndex);
    m_caster->GetMap()->ScriptsStart(sSpellScripts, uint32(m_spellInfo->Id | (effect->EffectIndex << 24)), m_caster, unitTarget);
}
...
void Spell::EffectActivateObject(SpellEffectEntry const* effect)
{
    ...
    int32 delay_secs = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectApplyGlyph(SpellEffectEntry const* effect)
{
    ...
    // apply new one
    if (uint32 glyph = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)))
    {
...
void Spell::EffectEnchantHeldItem(SpellEffectEntry const* effect)
{
    ...
    if (m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)))
    {
        uint32 enchant_id = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
{
    uint32 go_id = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectReputation(SpellEffectEntry const* effect)
{
    ...
    uint32 faction_id = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectQuestComplete(SpellEffectEntry const* effect)
{
    ...
    uint32 quest_id = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
    if (quest_id)
    {
...
void Spell::EffectSelfResurrect(SpellEffectEntry const* effect)
{
    ...
    if (damage < 0)
    {
        ...
        mana = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
    }
    // percent case
    else
    {
...
void Spell::EffectKnockBack(SpellEffectEntry const* effect)
{
    ...
    float speedxy = float(m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex))) * ratio;
...
{
    float speedxy = float(m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)))/10;
...
void Spell::EffectSendTaxi(SpellEffectEntry const* effect)
{
    ...
    unitTarget->ToPlayer()->ActivateTaxiPathTo(m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)),m_spellInfo->Id);
}
...
void Spell::EffectDispelMechanic(SpellEffectEntry const* effect)
{
    ...
    uint32 mechanic = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectDurabilityDamage(SpellEffectEntry const* effect)
{
    ...
    int32 slot = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
{
    uint32 name_id = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectStealBeneficialBuff(SpellEffectEntry const* effect)
{
    ...
    // Create dispel mask by dispel type
    uint32 dispelMask  = GetDispellMask(DispelType(m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex))));
    Unit::AuraMap const& auras = unitTarget->GetOwnedAuras();
    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        Aura * aura = itr->second;
...
void Spell::EffectKillCreditPersonal(SpellEffectEntry const* effect)
{
    ...
    unitTarget->ToPlayer()->KilledMonsterCredit(m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)), 0);
}
...
void Spell::EffectKillCredit(SpellEffectEntry const* effect)
{
    ...
    int32 creatureEntry = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
    if (!creatureEntry)
    {
...
void Spell::EffectQuestFail(SpellEffectEntry const* effect)
{
    ...
    unitTarget->ToPlayer()->FailQuest(m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)));
}
...
void Spell::EffectQuestStart(SpellEffectEntry const* effect)
{
    ...
    if (Quest const* qInfo = sObjectMgr->GetQuestTemplate(m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex))))
    {
...
void Spell::EffectActivateRune(SpellEffectEntry const* effect)
{
    ...
    {
        if (plr->GetRuneCooldown(j) && plr->GetCurrentRune(j) == RuneType(m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex))))
        {
...
void Spell::EffectCreateTamedPet(SpellEffectEntry const* effect)
{
    ...
    uint32 creatureEntry = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectDiscoverTaxi(SpellEffectEntry const* effect)
{
    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER)
        return;
    uint32 nodeid = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectWMOChange(SpellEffectEntry const* effect)
{
    if (gameObjTarget && gameObjTarget->GetGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)
    {
        ...
        int ChangeType = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
        switch (ChangeType)
        {
            case 0: // intact
...
void Spell::EffectPlayMusic(SpellEffectEntry const* effect)
{
    ...
    uint32 soundid = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex));
...
void Spell::EffectCastButtons(SpellEffectEntry const* effect)
{
    ...
    Player *p_caster = (Player*)m_caster;
    uint32 button_id = m_spellInfo->GetEffectMiscValue(SpellEffIndex(effect->EffectIndex)) + 132;
    uint32 n_buttons = m_spellInfo->GetEffectMiscValueB(SpellEffIndex(effect->EffectIndex));
