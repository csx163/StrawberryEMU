...
bool IsQuestTameSpell(uint32 spellId)
{
    ...
    if (!spellproto || !spellEffect0 || !spellEffect1)
        return false;
...
m_caster(Caster), m_spellValue(new SpellValue(m_spellInfo))
{
    ...
    if(m_spellInfo->GetDmgClass() == SPELL_DAMAGE_CLASS_MAGIC && !(m_spellInfo->AttributesEx2 & SPELL_ATTR2_CANT_REFLECTED))
    {
...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
    ...
    else if (m_damage > 0)
    {
        ...
        // Haunt
        if (classOptions && classOptions->SpellFamilyName == SPELLFAMILY_WARLOCK && classOptions->SpellFamilyFlags & 0x40000 && m_spellAura && m_spellAura->GetEffect(1))
        {
    ...
    else if (pushType)
    {
        ...
        if (!unitList.empty())
        {
            ...
            else
            {
                ...
                case 73058:
                    {
                        ...
                        unitList.sort(Strawberry::ObjectDistanceOrderPred(m_caster, false));
                ...
                // Death Pact
                if (adsClassOptions && adsClassOptions->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && adsClassOptions->SpellFamilyFlags & 0x00080000)
                {
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const * triggeredByAura)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "Spell::prepare: spell id %u source %u caster %d triggered %u mask %u", m_spellInfo->Id, m_caster->GetEntry(), m_originalCaster ? m_originalCaster->GetEntry() : -1, m_IsTriggeredSpell ? 1 : 0, m_targets.getTargetMask());
    //if (m_targets.getUnitTarget())
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (target)
    {
        ...
        if (!m_IsTriggeredSpell && auraRestriction && auraRestriction->TargetAuraStateNot && target->HasAuraState(AuraState(auraRestriction && auraRestriction->TargetAuraStateNot), m_spellInfo, m_caster))
        ...
        else if (m_caster == target)
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER) // Target - is player caster
            {
                ...
                if (m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_PALADIN && classOptions->SpellFamilyFlags & 0x0008000)
        ...
        */
        ...
        if (m_spellInfo->AttributesEx2 == SPELL_ATTR2_UNK20 && m_spellInfo->AttributesEx & SPELL_ATTR1_UNK9 && target->HasInArc(static_cast<float>(M_PI), m_caster)
            ...
            && (!(classOptions->SpellFamilyName == SPELLFAMILY_DRUID && classOptions->SpellFamilyFlags & 0x20000))
            //Mutilate no longer requires you be behind the target as of patch 3.0.3
            && (!(classOptions->SpellFamilyName == SPELLFAMILY_ROGUE && classOptions->SpellFamilyFlags & 0x200000))
            //Exclusion for Throw: Facing limitation was added in 3.2.x, but that shouldn't be
            && (!(classOptions->SpellFamilyName == SPELLFAMILY_ROGUE && classOptions->SpellFamilyFlags & 0x00000001)))
        {
            SendInterrupted(2);
