...
void WorldSession::HandleMailReturnToSender(WorldPacket & recv_data)
{
    ...
    if (m->messageType == MAIL_NORMAL && m->sender)
    {
        ...
        draft.AddMoney(m->money).SendReturnToSender(GetAccountId(),m->sender, m->receiver);
    }
    ...
    if (msg == EQUIP_ERR_OK)
    {
        ...
        if (m->COD > 0)                                     //if there is COD, take COD money from player and send them to sender by mail
        {
            ...
            if (receive || sender_accId)
            {
                MailDraft(m->subject, "")
                    .AddMoney(m->COD)
                    .SendMailTo(trans, MailReceiver(receive, m->sender), MailSender(MAIL_NORMAL, uint32(m->receiver)), MAIL_CHECK_MASK_COD_PAYMENT);
            }
