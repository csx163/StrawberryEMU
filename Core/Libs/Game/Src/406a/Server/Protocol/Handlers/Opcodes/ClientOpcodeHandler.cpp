...
static void DefineClientOpcode(uint32 opcode, const char* name, SessionStatus status, PacketProcessing packetProcessing, void (WorldSession::*handler)(WorldPacket& recvPacket))
{
...
void ClientOpcodeTable()
{
    ...
    CLIENTOPCODE(CMSG_GUILD_SET_NOTE,                  STATUS_LOGGEDIN, PROCESS_THREADUNSAFE, &WorldSession::HandleNULL );
    CLIENTOPCODE(CMSG_GUILD_INFO_TEXT,                 STATUS_LOGGEDIN, PROCESS_THREADUNSAFE, &WorldSession::HandleNULL );
    ...
    // Player Arena
    CLIENTOPCODE(CMSG_ARENA_TEAM_CREATE,               STATUS_NEVER,    PROCESS_INPLACE,      &WorldSession::HandleArenaTeamCreate );
    ...
    CLIENTOPCODE(CMSG_TAXISHOWNODES,                     STATUS_NEVER,    PROCESS_INPLACE,      &WorldSession::HandleTaxiQueryAvailableNodes   );
