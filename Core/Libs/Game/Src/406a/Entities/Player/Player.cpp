...
bool Player::BuildEnumData(QueryResult result, WorldPacket* data)
{
    ...
    *data << fields[12].GetFloat();                         // z
    ...
    if (uint32 guild_id = fields[13].GetUInt32())
        *data << uint64(MAKE_PAIR64(HIGHGUID_GUILD, fields[13].GetUInt32()));              // guild id
    else
        *data << uint64(0);
...
bool Player::LoadFromDB(uint32 guid, SQLQueryHolder *holder)
{
    ...
    //"health, power1, power2, power3, power4, power5, power6, power7, power8, power9, power10, power11, instance_id, speccount, activespec, exploredZones, equipmentCache, knownTitles, achievementPoints, actionBars FROM characters WHERE guid = '%u'", guid);
    ...
    if (!isGameMaster() && ar)
    {
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                ...
                else if (missingAchievement) // if (missingAchievement) covered by this case
...
void Player::learnQuestRewardedSpells(Quest const* quest)
{
    ...
    uint32 learned_0 = spellEffect->EffectTriggerSpell;
    if (sSpellMgr->GetSpellRank(learned_0) > 1 && !HasSpell(learned_0))
    {
        ...
        if (!learnedInfo || !learnedSpellEffect0 || !learnedSpellEffect1)
            return;
        ...
        for (SpellsRequiringSpellMap::const_iterator itr2 = spellsRequired.first; itr2 != spellsRequired.second; ++itr2)
        {
            ...
            if (learnedSpellEffect0 && learnedSpellEffect0->Effect == SPELL_EFFECT_TRADE_SKILL && learnedSpellEffect1 && learnedSpellEffect1->Effect == 0)
            {
                ...
                for (PlayerSpellMap::const_iterator itr = m_spells.begin(); itr != m_spells.end(); ++itr)
                {
                    ...
                    if (!itrSpellEffect0 || !itrSpellEffect1)
                        continue;

                    if ((itrSpellEffect0->Effect != SPELL_EFFECT_TRADE_SKILL) || (itrSpellEffect1->Effect != 0))
                        continue;
...
bool Player::CanNoReagentCast(SpellEntry const* spellInfo) const
{
    ...
    if (classOptions->SpellFamilyFlags)
        return true;
...
void Player::SendRefundInfo(Item *item)
{
    ...
    for (int i = 0; i < MAX_ITEM_EXT_COST_ITEMS; ++i)                      // item cost data
    {
