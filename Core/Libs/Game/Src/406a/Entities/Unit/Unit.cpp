...
void Unit::RemoveAurasDueToSpellByDispel(uint32 spellId, uint64 casterGUID, Unit *dispeller)
{
    for (AuraMap::iterator iter = m_ownedAuras.lower_bound(spellId); iter != m_ownedAuras.upper_bound(spellId);)
    {
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            ...
            // Unstable Affliction (crash if before remove aura?)
...
void Unit::RemoveAurasWithFamily(SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint32 familyFlag3, uint64 casterGUID)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        if (!casterGUID || aura->GetCasterGUID() == casterGUID)
        {
            ...
            if (spell->GetSpellFamilyName() == uint32(family) && classOptions->SpellFamilyFlags == (familyFlag1 || familyFlag2 || familyFlag3))
            {
                RemoveAura(iter);
                continue;
            }
...
bool Unit::HandleDummyAuraProc(Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const * procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    switch(dummySpell->GetSpellFamilyName())
    {
            ...
            switch(dummySpell->Id)
            {
                ...
                case 56372:
                {
                    ...
                    for (SpellCooldowns::const_iterator itr = SpellCDs.begin(); itr != SpellCDs.end(); itr++)
                    {
                        ...
                        SpellClassOptionsEntry const* classOptions = SpellCDs_entry->GetSpellClassOptions();
                        if (!classOptions)
                            continue;

                        // Frost Nova
                        if (SpellCDs_entry->GetSpellFamilyName() == SPELLFAMILY_MAGE && classOptions->SpellFamilyFlags & 0x00000040)
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (classOptions->SpellFamilyFlags == 0 && dummySpell->SpellIconID == 1932)
            {
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 55680:
                {
                    ...
                    SpellEffectEntry const* spellEffect = GoPoH->GetSpellEffect(SpellEffIndex(effIndex));
                    if (!GoPoH || !spellEffect)
                        return false;
                    ...
                    int32 tickcount = GetSpellMaxDuration(GoPoH) / spellEffect->EffectAmplitude;
                    if (!tickcount)
                        return false;
                ...
                case 70770:
                    ...
                    if (classOptions->SpellFamilyFlags & 0x800)
                    {
                        ...
                        SpellEffectEntry const* spellEffect = blessHealing->GetSpellEffect(SpellEffIndex(EFFECT_0));
                        if (!blessHealing || !spellEffect)
                            return false;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (classOptions->SpellFamilyFlags & 0x80000)
            {
                ...
                {
                    if ((procSpell->GetSpellFamilyName() == SPELLFAMILY_PALADIN) && (classOptions->SpellFamilyFlags & 0x40000000))
                    {
            ...
            switch(dummySpell->Id)
            {
                ...
                case 63280: // Glyph of Totem of Wrath
                {
                    ...
                    for (AuraEffectList::const_iterator i = auras.begin(); i != auras.end(); ++i)
                    {
                        ...
                        if (spell->GetSpellFamilyName() == uint32(SPELLFAMILY_SHAMAN) && classOptions->SpellFamilyFlags == 0x02000000)
                        {
        ...
        switch (auraSpellInfo->GetSpellFamilyName())
        {
            ...
            case SPELLFAMILY_HUNTER:
            {
                if (auraSpellInfo->SpellIconID == 3247)     // Piercing Shots
                {
                    ...
                    SpellEffectEntry const* spellEffect = TriggerPS->GetSpellEffect(SpellEffIndex(EFFECT_0));
                    if (!TriggerPS || !spellEffect)
                        return false;

                    basepoints0 = CalculatePctN(int32(damage), triggerAmount) / (GetSpellMaxDuration(TriggerPS) / spellEffect->EffectAmplitude);
...
uint32 Unit::SpellDamageBonus(Unit *pVictim, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (!spellProto || !classOptions || !pVictim || damagetype == DIRECT_DAMAGE)
        return pdamage;
    ...
    if (spellProto)
    {
        ...
        {
            SpellEquippedItemsEntry const* equippedItems = (*i)->GetSpellProto()->GetSpellEquippedItems();
            if (!equippedItems)
                continue;

            if ((*i)->GetMiscValue() & GetSpellSchoolMask(spellProto))
            {
                if (equippedItems->EquippedItemClass == -1)
                {
                    AddPctN(DoneTotalMod, (*i)->GetAmount());
                }
                else if (!((*i)->GetSpellProto()->AttributesEx5 & SPELL_ATTR5_SPECIAL_ITEM_CLASS_CHECK))
                {
                    if (equippedItems->EquippedItemClass & equippedItems->EquippedItemClass)
...
int32 Unit::SpellBaseDamageBonus(SpellSchoolMask schoolMask)
{
    ...
    for (AuraEffectList::const_iterator i = mDamageDone.begin(); i != mDamageDone.end(); ++i)
    {
        ...
        equippedItems->EquippedItemClass == -1 &&
                                                            // -1 == any item class (not wand then)
        equippedItems->EquippedItemInventoryTypeMask == 0)
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit *pVictim, Aura * aura, SpellEntry const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const *& spellProcEvent)
{
    ...
    if (!isVictim && GetTypeId() == TYPEID_PLAYER)
    {
        if (spellProto->GetEquippedItemClass() == ITEM_CLASS_WEAPON)
        {
            ...
            if (!item || item->IsBroken() || item->GetProto()->Class != ITEM_CLASS_WEAPON || !((1<<item->GetProto()->SubClass) & equippedItems->EquippedItemSubClassMask))
                return false;
        }
        else if (equippedItems->EquippedItemClass == ITEM_CLASS_ARMOR)
        {
            // Check if player is wearing shield
            Item *item = this->ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
            if (!item || item->IsBroken() || item->GetProto()->Class != ITEM_CLASS_ARMOR || !((1<<item->GetProto()->SubClass) & equippedItems->EquippedItemSubClassMask))
                return false;
        }
    }
...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffect *triggeredByAura)
{
    ...
    if (!(spellProto->GetSpellFamilyName() == SPELLFAMILY_PRIEST && classOptions->SpellFamilyFlags & 0x20))
    {
