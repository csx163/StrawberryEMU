...
void RealmDBConnection::DoPrepareStatements()
{
    if (!m_reconnecting)
    ...
    PREPARE_STATEMENT(LOGIN_SET_IPAUTOBANNED, "INSERT INTO ip_banned VALUES (?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()+?, 'Strawberry RealmServer', 'Failed login autoban')", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(LOGIN_SET_ACCAUTOBANNED, "INSERT INTO account_banned VALUES (?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()+?, 'Strawberry RealmServer', 'Failed login autoban', 1)", CONNECTION_ASYNC)
